...
import com.zina.microservices.core.review.persistence.ReviewRepository;
...
public class ReviewServiceApplicationTests {
	...
	private WebTestClient.BodyContentSpec getAndVerifyReviewsByProductId(String productIdQuery,
		...
		return client.get().uri("/review" + productIdQuery).accept(APPLICATION_JSON_UTF8).exchange().expectStatus()
	...
	private WebTestClient.BodyContentSpec postAndVerifyReview(int productId, int reviewId, HttpStatus expectedStatus) {
		Review review = new Review(productId, reviewId, "Author " + reviewId, "Subject " + reviewId,
				"Content " + reviewId, "SA");
		return client.post().uri("/review").body(just(review), Review.class).accept(APPLICATION_JSON_UTF8).exchange()
	...
	private WebTestClient.BodyContentSpec deleteAndVerifyReviewsByProductId(int productId, HttpStatus expectedStatus) {
		return client.delete().uri("/review?productId=" + productId).accept(APPLICATION_JSON_UTF8).exchange()
