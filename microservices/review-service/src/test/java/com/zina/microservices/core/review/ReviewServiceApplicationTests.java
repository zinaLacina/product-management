...
public class ReviewServiceApplicationTests {
	...
	public void getReviewsByProductId() {
		...
		client.get().uri("/review?productId=" + productId).accept(APPLICATION_JSON_UTF8).exchange().expectStatus()
				.isOk().expectHeader().contentType(APPLICATION_JSON_UTF8).expectBody().jsonPath("$.length()")
				.isEqualTo(3).jsonPath("$[0].productId").isEqualTo(productId);
	}
	...
	public void getReviewsInvalidParameter() {
		...
		client.get().uri("/review?productId=no-integer").accept(APPLICATION_JSON_UTF8).exchange().expectStatus()
				.isEqualTo(BAD_REQUEST).expectHeader().contentType(APPLICATION_JSON_UTF8).expectBody()
				.jsonPath("$.path").isEqualTo("/review").jsonPath("$.message").isEqualTo("Type mismatch.");
	}
