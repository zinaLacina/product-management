...
import com.zina.api.composite.product.ProductAggregate;
import com.zina.api.composite.product.RecommendationSummary;
import com.zina.api.composite.product.ReviewSummary;
...
public class ProductCompositeServiceApplicationTests {
	...
	private WebTestClient.BodyContentSpec getAndVerifyProduct(int productId, HttpStatus expectedStatus) {
		return client.get().uri("/product-composite/" + productId).accept(APPLICATION_JSON_UTF8).exchange()
				.expectStatus().isEqualTo(expectedStatus).expectHeader().contentType(APPLICATION_JSON_UTF8)
				.expectBody();
	}

	private void postAndVerifyProduct(ProductAggregate compositeProduct, HttpStatus expectedStatus) {
		client.post().uri("/product-composite").body(just(compositeProduct), null).exchange().expectStatus()
